...
class LinearConvBlock(eqx.Module):
    norm: eqx.nn.GroupNorm
    conv: eqx.nn.Conv
    def __init__(self, *,  dims, kernel_size=3, padding_mode = 'REPLICATE', min_group_norm_channel = 32, in_channels, out_channels, key = jr.PRNGKey(0)):
        key1, key2, key3 = jr.split(key, 3)
...
class SimpleSelfAttention(eqx.Module):
    ...
    def __init__(self, *, in_channels, spatial_dims, key=jr.PRNGKey(0), heads=4, dim_head=32):
        """
        Initializes the attention mechanism for N-dimensional inputs.

        Args:
            dim (int): Number of input channels.
            spatial_dims (int): The number of spatial dimensions (1D, 2D, 3D).
        ...
        self.group_norm = eqx.nn.GroupNorm(min(in_channels // 4, 32), in_channels)
        self.heads = heads
        hidden_dim = dim_head * heads
        
        # Define convolutional layers for arbitrary spatial dimensions
        self.to_qkv = eqx.nn.Conv(spatial_dims, in_channels, hidden_dim * 3, kernel_size=1, key=keys[0])
        self.to_out = eqx.nn.Conv(spatial_dims, hidden_dim, in_channels, kernel_size=1, key=keys[1])
    ...
    def __call__(self, x):
        ...
        Args:
            x (jnp.ndarray): Input tensor of shape (C, spatial_dims...)
        
        Returns:
            jnp.ndarray: Output tensor of the same shape as `x`
        """
        # Get input shape (C, spatial...)
        spatial_shape = x.shape[1:]  # Shape without channels
        ...
        # Reshape into (qkv, heads, channels_per_head, spatial_dim_product)
        q, k, v = rearrange(qkv, f"(qkv heads c) {' '.join(['s' + str(i) for i in range(self.spatial_dims)])} -> qkv heads c ({' '.join(['s' + str(i) for i in range(self.spatial_dims)])})",
                            heads=self.heads, qkv=3)
        
        # Apply softmax to keys (reducing complexity)
        k = jax.nn.softmax(k, axis=-1)

        # Compute context (efficient attention computation)
        ...
        out = rearrange(out, f"heads c ({' '.join(['s' + str(i) for i in range(self.spatial_dims)])}) -> (heads c) {' '.join(['s' + str(i) for i in range(self.spatial_dims)])}", heads=self.heads, **{f"s{i}": spatial_shape[i] for i in range(self.spatial_dims)})

        return self.to_out(out)

class ResnetBlockAttention(eqx.Module):
    conv_block1: ConvBlock
    conv_block2: ConvBlock
    conv: eqx.nn.Conv
    linear: eqx.nn.Linear
    ...
    norm: eqx.nn.GroupNorm
    def __init__(self, *, dims = 2, kernel_size=3, padding_mode = 'REPLICATE', min_group_norm_channel = 32, in_channels, out_channels, embed_dim=256, heads=4, dim_head=32, key = jr.PRNGKey(0)):
        key1, key2, key3, key4, key5 = jr.split(key, 5)
        self.conv_block1 = ConvBlock(dims = dims, kernel_size=kernel_size, padding_mode = padding_mode, min_group_norm_channel = min_group_norm_channel, in_channels = in_channels, out_channels = out_channels, key = key1)
        self.conv_block2 = ConvBlock(dims = dims, kernel_size=kernel_size, padding_mode = padding_mode, min_group_norm_channel = min_group_norm_channel, in_channels = out_channels, out_channels = out_channels, key = key2)
        self.conv = eqx.nn.Conv(dims, in_channels, out_channels, kernel_size = 1, key = key3)
        self.linear = eqx.nn.Linear(embed_dim, out_channels, key=key4)
        self.attention = SimpleSelfAttention(in_channels = out_channels, spatial_dims = dims, key = key5, heads=heads, dim_head=dim_head)
        self.norm = eqx.nn.GroupNorm(min(in_channels//4, min_group_norm_channel), in_channels)
    def __call__(self, t, x):
        h = self.conv_block1(x)
        t = self.linear(jax.nn.silu(t))
        h = h + jnp.expand_dims(t, axis=tuple(range(1, h.ndim)))
        h = self.conv_block2(h)
        h = h + self.conv(self.norm(x))
        h = h + self.attention(h) 
        return h
    
class ResnetBlockDownAttention(eqx.Module):
    norm1: eqx.nn.GroupNorm
    conv1: ConvBlock
    down: eqx.nn.Conv
    conv_block2: ConvBlock
    ...
    attention: SimpleSelfAttention
    def __init__(self, *, dims = 2, kernel_size=3, padding_mode = 'REPLICATE', min_group_norm_channel = 32, in_channels, out_channels, embed_dim=256, heads=4, dim_head=32, key = jr.PRNGKey(0)):
        key0, key1, key2, key3, key4, key5 = jr.split(key, 6)
        padding = smoothing_padding(dims, kernel_size)
        self.down = eqx.nn.Conv(dims, in_channels, in_channels, kernel_size=3, stride=2, padding=1, key=key0, padding_mode=padding_mode)
        self.norm1 = eqx.nn.GroupNorm(min(in_channels//4, min_group_norm_channel), in_channels)
        self.conv1 = eqx.nn.Conv(dims, in_channels, out_channels, kernel_size=kernel_size, padding=padding, padding_mode=padding_mode, key=key1)
...
class ResnetBlockUpAttention(eqx.Module):
    norm1: eqx.nn.GroupNorm
    conv1: ConvBlock
    up: eqx.nn.ConvTranspose
    conv_block2: ConvBlock
    ...
    linear: eqx.nn.Linear
    attention: SimpleSelfAttention
    def __init__(self, *, dims = 2, kernel_size=3, padding_mode = 'REPLICATE', padding_mode_up = 'ZEROS', min_group_norm_channel = 32, in_channels, out_channels, embed_dim=256, heads=4, dim_head=32, key = jr.PRNGKey(0)):
        key0, key1, key2, key3, key4, key5 = jr.split(key, 6)
        padding = smoothing_padding(dims, kernel_size)
        if padding_mode_up == 'CIRCULAR':
            padding_up = 'SAME'
...
class AttentionUNet(eqx.Module):
    ...
    mlp: eqx.nn.MLP
    conv: eqx.nn.Conv
    downblock: list[ResnetBlockDownAttention]
    upblock: list[ResnetBlockUpAttention]
    upresblock: list[ResnetBlockAttention]
    downfeatureblock: list[ResnetBlockAttention]
    upfeatureblock: list[ResnetBlockAttention]
    downfeatureblock: list[ResnetBlockAttention]
    upblockbefore: list[list[ResnetBlockAttention]]
    upblockafter: list[list[ResnetBlockAttention]]
    downblockbefore: list[list[ResnetBlockAttention]]
    downblockafter: list[list[ResnetBlockAttention]]
    midblock: list[ResnetBlockAttention]
    upfeatureblock_reduce: list[ResnetBlockAttention]
    finalblock: list[ResnetBlockAttention]
    resblock: ResnetBlockAttention
    block: ConvBlock
    def __init__(self, *, context_channels=0, heads=4, dim_head=32, padding_mode = 'REPLICATE',   beforeblock_length = 0, afterblock_length = 0, midblock_length = 2, final_block_length = 0, kernel_size = 3, data_shape, features=[32, 64, 128], downscaling_factor=2, key=jr.PRNGKey(0)):
        in_channels = data_shape[0]
        out_channels = in_channels - context_channels
        dims = len(data_shape) - 1
        padding_mode_up = 'ZEROS'
        ...
        for i in range(len(features)-1):
            subkeys = jr.split(keys[2 + i], 8 + 2 * (beforeblock_length + afterblock_length))
            self.downfeatureblock.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[i+1], padding_mode = padding_mode, in_channels=features[i], out_channels=features[i+1], embed_dim=emb_dim, key=subkeys[1]))
            self.upfeatureblock.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head,kernel_size = kernel_size[i+1], padding_mode = padding_mode, in_channels= 2*features[i+1], out_channels=features[i+1], embed_dim=emb_dim, key=subkeys[2]))
            self.upfeatureblock_reduce.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head,kernel_size = kernel_size[i+1], padding_mode = padding_mode, in_channels=features[i+1], out_channels=features[i], embed_dim=emb_dim, key=subkeys[3]))
            downresblock_before = []
            upresblock_before = []
            for j in range(beforeblock_length):
                key1, key2 = jr.split(subkeys[3 + j])
                downresblock_before.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head,kernel_size = kernel_size[i+1],padding_mode = padding_mode, in_channels=features[i+1], out_channels=features[i+1], embed_dim=emb_dim, key=key1))
                upresblock_before.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head,kernel_size = kernel_size[i+1], padding_mode = padding_mode,in_channels=2*features[i+1], out_channels=features[i+1], embed_dim=emb_dim, key=key2))
            self.downblockbefore.append(downresblock_before)
            self.upblockbefore.append(upresblock_before)
            downresblock_after = []
            upresblock_after = []
            for j in range(afterblock_length):
                key1, key2 = jr.split(subkeys[3 + 2*beforeblock_length + j])
                downresblock_after.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[i+1], padding_mode = padding_mode, in_channels=features[i+1], out_channels=features[i+1], embed_dim=emb_dim, key=key1))
                upresblock_after.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[i+1], padding_mode = padding_mode, in_channels=2*features[i+1], out_channels=features[i+1], embed_dim=emb_dim, key=key2))
            self.downblockafter.append(downresblock_after)
            self.upblockafter.append(upresblock_after)
            self.downblock.append(ResnetBlockDownAttention(dims=dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[i+1], padding_mode = padding_mode, in_channels=features[i+1], out_channels=features[i+1], embed_dim=emb_dim, key=subkeys[3 + 2 * (beforeblock_length + afterblock_length) + 2]))
            self.upblock.append(ResnetBlockUpAttention(dims=dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[i+1], padding_mode = padding_mode, padding_mode_up = padding_mode_up, in_channels=features[i+1], out_channels=features[i+1], embed_dim=emb_dim, key=subkeys[3 + 2 * (beforeblock_length + afterblock_length) + 3]))
            self.upresblock.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[i+1], padding_mode = padding_mode, in_channels= 2*features[i+1], out_channels=features[i+1], embed_dim=emb_dim, key=subkeys[3 + 2 * (beforeblock_length + afterblock_length) + 4]))
        self.midblock = []
        midblock_keys = jr.split(keys[2*len(features)+3], midblock_length)
        for i in range(midblock_length):
            self.midblock.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[0], padding_mode = padding_mode, in_channels=features[-1], out_channels=features[-1], embed_dim=emb_dim, key=midblock_keys[i]))
        self.finalblock = []
        finalblock_keys = jr.split(keys[2*len(features)+4], final_block_length)
        for i in range(final_block_length):
            self.finalblock.append(ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[0], padding_mode = padding_mode, in_channels=features[0], out_channels=features[0], embed_dim=emb_dim, key=finalblock_keys[i]))
        self.resblock = ResnetBlockAttention(dims=dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[0], padding_mode = padding_mode, in_channels=2*features[0], out_channels=features[0], embed_dim=emb_dim, key=keys[2*len(features)+1])
        # self.block = ResnetBlockAttention(dims = dims, heads=heads, dim_head=dim_head, kernel_size = kernel_size[0], padding_mode = padding_mode, in_channels = features[0], out_channels = out_channels, embed_dim=emb_dim,  key=keys[2*len(features)+2])
        # self.block = ConvBlock(dims = dims, kernel_size = kernel_size[0], padding_mode = padding_mode, in_channels = features[0], out_channels = out_channels, key=keys[2*len(features)+2])
        self.block = LinearConvBlock(dims = dims, kernel_size = kernel_size[0], padding_mode = padding_mode, in_channels = features[0], out_channels = out_channels, key=keys[2*len(features)+2])
        # self.block = eqx.nn.Conv(dims, features[0], out_channels, kernel_size=kernel_size[0], padding=padding, padding_mode = padding_mode, key=keys[2*len(features)+1])
    @eqx.filter_jit
    def __call__(self, t, y):
        # Lift 
        t = self.gfp(t)  # 1 -> features[0]
